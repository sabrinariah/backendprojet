...
public class UserController {
    ...
    public ResponseEntity<String> addUser(@RequestBody UserRequest userRequest) {
        ...
        try {
            ...
            // Si utilisateur existe déjà, on log et continue
        ...
        // 🔹 Message clair selon situation
        if (keycloakCreated) {
            return ResponseEntity.ok("Utilisateur créé avec succès dans Keycloak et DB !");
        } else {
            return ResponseEntity.ok("Utilisateur déjà existant dans Keycloak mais créé dans DB avec succès !");
        }
    }
    ...
    public ResponseEntity<?> getKeycloakUsers() {
        try {
            ...
            // 🔹 Log simple du message de Keycloak
            System.err.println("Erreur Keycloak 400 Bad Request : " + bre.getMessage());
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body("Erreur Keycloak 400 Bad Request : " + bre.getMessage());
        } catch (Exception e) {
            e.printStackTrace();
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
    ...
    public ResponseEntity<String> updateUser(@PathVariable String username, @RequestBody CreateUserRequest request) {
        try {
            ...
            userService.updateUserKeycloak(username, request); // mettre à jour DB si besoin
    ...
    public ResponseEntity<Boolean> checkUserExists(@PathVariable String username) {
        try {
            // Récupère tous les utilisateurs depuis Keycloak via ton service
            List<Map<String, Object>> users = keycloakService.getAllUsersWithRoles();
    ...
    public ResponseEntity<List<String>> getExistingUsers() {
        try {
            ...
            // Filtre et récupère uniquement les usernames
